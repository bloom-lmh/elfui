...
Pass an inline template literal directly, or combine imported stylesheet strings as generated by the scaffold:
...
`html` and `css` tagged-template helpers were removed in beta.7. Pass inline template literals directly to `defineHtml()` and `defineStyle()`.
