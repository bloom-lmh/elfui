...
export type FragmentPropsSource = () => unknown;

const fragmentPropsObject = (value: unknown): Record<PropertyKey, unknown> | null =>
  (typeof value === "object" && value !== null) || typeof value === "function"
    ? (value as Record<PropertyKey, unknown>)
    : null;

/**
 * Creates the readonly props view used by a named Fragment.
 *
 * Sources stay lazy so a binding such as `:label=${label}` reads the parent
 * state inside the Fragment's DOM effect instead of capturing an initial
 * snapshot. Later sources override earlier sources, matching object-spread
